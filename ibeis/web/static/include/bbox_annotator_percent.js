...
(function() {
  ...
  BBoxSelector = (function() {
    function BBoxSelector(image_frame, options, default_color) {
      ...
      this.selector = $('<div class="ia-bbox-selector"></div>');
      this.selector.css({
    ...
    BBoxSelector.prototype.create_label_box = function(options) {
      ...
      this.label_box = $('<div class="ia-label-box"></div>');
      this.label_box.css({
        "position": "absolute"
      });
      ...
      switch (options.input_method) {
        case 'select':
          ...
          this.label_input = $('<select class="ia-label-input" name="label"></select>');
        ...
        case 'text':
          ...
          this.label_input = $('<input class="ia-label-input" name="label" ' + 'type="text" value>');
          this.label_box.append(this.label_input);
        ...
        case 'fixed':
          ...
          this.label_input = $('<input class="ia-label-input" name="label" type="text">');
          this.label_box.append(this.label_input);
    ...
    BBoxSelector.prototype.rectangle = function() {
      var rect, x1, x2, y1, y2, width, height;
      ...
      width = $('.ia-image-frame').width();
      height = $('.ia-image-frame').height();
      return rect = {
        left:   100.0 * (x1) / width,
        top:    100.0 * (y1) / height,
        width:  100.0 * (x2 - x1 + 1) / width,
        height: 100.0 * (y2 - y1 + 1) / height
      };
    };
  ...
  this.BBoxAnnotator = (function() {
    function BBoxAnnotator(options) {
      ...
      annotator = this;
      this.annotator_element = $(options.id || "#ia-bbox-annotator");
      this.border_width = options.border_width || 2;
      this.show_label = options.show_label || (options.input_method !== "fixed");
      this.image_frame = $('<div class="ia-image-frame"></div>');
    ...
    BBoxAnnotator.prototype.handle_resize = function() {
      var annotator;
      annotator = this

      var w1, h1, w2, h2;
      w1 = annotator.image_element.width
      h1 = annotator.image_element.height
      w2 = annotator.image_frame.width();
      var target_height = ( (w2 / w1) * h1 );
          ...
          case 'free':
            ...
            if(active_box.length > 0)
            {
              ...
              if(e.which === 27 || e.which === 75 || e.which === 8)
              {
                ...
                {
                  annotator.target = false;
                  if(annotator.editing_mouse_inside)
                  {
...
//                 temp.trigger('mouseenter');
    ...
    BBoxAnnotator.prototype.add_entry = function(entry) {
      ...
      function update_style_hover(annotator, e)
      {
        if(! annotator.target)
        {
      ...
      function update_style_editing(annotator, e)
      {
        if((annotator.adding || ! annotator.editing_mouse_inside) && ! annotator.target)
        {
          box_element.css('outline', 'none');
      ...
      function update_style_target(annotator, e)
      {
      ...
      {
        var img_width = parseFloat(annotator.image_frame.css('width')) - 2.0;
        var img_height = parseFloat(annotator.image_frame.css('height')) - 2.0;
      ...
      var resize_params = {
          ...
          stop: function(e, ui) {
            ...
            annotator.editing = false;
            update_dimensions();
            update_style_editing(annotator, e);
          },
          containment: "#ia-bbox-annotator",
      ...
      {
        text_box.text('');
        $('.ia-label-text-box').css('padding', '0px');
      }

      annotator.editing_mouse_inside = false;

      box_element.mouseup(function(e) {
        console.log('MOUSE UP ' + annotator.hit_menuitem + ' ' + annotator.moving + ' ' + annotator.adding + ' ' + annotator.editing);
        if (e.which === 1) {
          if( ! annotator.moving && ! annotator.adding)
          {
            if(annotator.target)
            {
              console.log('UN-TARGET');
              annotator.target = false;
              update_style_hover(annotator, e);
            }
            else
            {
              console.log('TARGET');
              annotator.target = true;
              update_style_target(annotator, e);
            }
          }
        }
        annotator.adding = false;
        annotator.moving = false;
      })
    ...
    BBoxAnnotator.prototype.clear_all = function(e) {
      $(".ia-annotated-bounding-box").detach();
