...
from ibeis.plottool import custom_constants  # NOQA
import six
from matplotlib import colors as mcolors
import colorsys
import numpy as np  # NOQA
import utool as ut
#from ibeis.plottool import colormaps as cmaps2
...
def ensure_base01(color):
    ...
    CommandLine:
        python -m ibeis.plottool.color_funcs ensure_base01

    Example:
        >>> # ENABLE_DOCTEST
        >>> from ibeis.plottool.color_funcs import *  # NOQA
        >>> ensure_base01('g')
...
def testshow_colors(rgb_list, gray=ut.get_argflag('--gray')):
    ...
    CommandLine:
        python -m ibeis.plottool.color_funcs testshow_colors --show
...
def desaturate_rgb(rgb, amount):
    r"""
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-desaturate_rgb --show
...
def lighten_rgb(rgb, amount):
    r"""
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-lighten_rgb --show
        python -m ibeis.plottool.color_funcs --test-lighten_rgb
...
def adjust_hsv_of_rgb255(rgb255, *args, **kwargs):
    """
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-adjust_hsv_of_rgb255 --show
...
def adjust_hsv_of_rgb(rgb, hue_adjust=0.0, sat_adjust=0.0, val_adjust=0.0):
    ...
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-adjust_hsv_of_rgb --show
...
def distinct_colors(N, brightness=.878, randomize=True, hue_range=(0.0, 1.0), cmap_seed=None):
    ...
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 2 --show --hue-range=0.05,.95
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 3 --show --hue-range=0.05,.95
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 4 --show --hue-range=0.05,.95
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 3 --show --no-randomize
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 4 --show --no-randomize
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 6 --show --no-randomize
        python -m ibeis.plottool.color_funcs --test-distinct_colors --N 20 --show

    References:
        http://blog.jianhuashao.com/2011/09/generate-n-distinct-colors.html

    CommandLine:
        python -m ibeis.plottool.color_funcs --exec-distinct_colors --show
        python -m ibeis.plottool.color_funcs --exec-distinct_colors --show --no-randomize --N 50
        python -m ibeis.plottool.color_funcs --exec-distinct_colors --show --cmap_seed=foobar
...
def show_all_colormaps():
    ...
    CommandLine:
        python -m ibeis.plottool.color_funcs --test-show_all_colormaps --show
        python -m ibeis.plottool.color_funcs --test-show_all_colormaps --show --type=Miscellaneous
        python -m ibeis.plottool.color_funcs --test-show_all_colormaps --show --cmap=RdYlBu
...
if __name__ == '__main__':
    """
    CommandLine:
        python -m ibeis.plottool.color_funcs
        python -m ibeis.plottool.color_funcs --allexamples
        python -m ibeis.plottool.color_funcs --allexamples --noface --nosrc
    """
