...
def TEST_IBS_GETTERS(ibs=None):
    ...
    #id_iter = fid_list
    #id_colname = 'rowid'
    #desc_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_sifts',), fid_list, num_params=len(fid_list), eager=eager)
    #"""
    #%timeit list(ibs.dbcache.get(FEATURE_TABLE, ('feature_sifts',), fid_list, num_params=len(fid_list), eager=False))
    #%timeit ibs.dbcache.get(FEATURE_TABLE, ('feature_sifts',), fid_list, num_params=len(fid_list), eager=True)
    #"""
    #utool.embed()
