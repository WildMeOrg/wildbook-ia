...
def TEST_NEW_SCHEMA(ibs):
    ...
    aid_list2 = ibs.add_annotations(gid_list, bbox_list2)
    print('[TEST] aid_list2 = %r' % aid_list2,)
    nid_list = ibs.get_annotation_nids(aid_list, distinguish_unknowns=False)
    print('[TEST] nid_list = %r' % nid_list,)

    gt_list = ibs.get_annotation_groundtruth(aid_list)
    gt_list2 = ibs.get_annotation_groundtruth(aid_list2)
