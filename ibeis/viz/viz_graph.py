...
def test_qt_graphs(**kwargs):
    r"""
    CommandLine:
        python -m ibeis.viz.viz_graph test_qt_graphs --show
    ...
    class AnnotGraphWidget(gt.GuitoolWidget):
        def initialize(self, infr=None, use_image=False):
            ...
            self.vsplitter = self.addNewSplitter(orientation=Qt.Vertical)
            self.mpl_wgt = MatplotlibWidget(parent=self)
            self.vsplitter.addWidget(self.mpl_wgt)
            self.control_panel = self.vsplitter.addNewWidget(orientation=Qt.Vertical,
                                                             vertical_stretch=1, margin=1, spacing=1)
            bbar1 = self.bbar1 = self.control_panel.addNewWidget(orientation=Qt.Horizontal,
                                                                 margin=1, spacing=1)
            bbar2 = self.bbar2 = self.control_panel.addNewWidget(orientation=Qt.Horizontal,
                                                                 margin=1, spacing=1)
            bbar3 = self.bbar3 = self.control_panel.addNewWidget(orientation=Qt.Horizontal,
                                                                 margin=1, spacing=1)
            ...
            bbar2.addNewButton('Cut', pressed=self.cut)

            bbar2.addNewButton('Params', pressed=self.edit_config)

            bbar2.addNewButton('Deselect', pressed=self.unselect_all)
            ...
            bbar2.addNewButton('Reset', pressed=self.reset)

            self.cb = None

            def _debug_button(func, refresh=True):
                def _simple_onevent():
                    func()
                    if refresh:
                        self.show_page()
            ...
            #Debug row
            _debug_button(self.infr.apply_mst)
            _debug_button(self.infr.apply_scores)
            _debug_button(self.infr.apply_feedback)
            _debug_button(self.infr.apply_weights)
            _debug_button(self.infr.apply_cuts)
            _debug_button(self.infr.apply_all)
            _debug_button(self.embed)

            self.mpl_wgt.click_inside_signal.connect(self.on_click_inside)

            #bbar.addNewButton(self.plot_weights, 0)
            #bbar.addNewButton(self.print_weights, 0)
        ...
        @property
        def ax(self):
            return self.mpl_wgt.ax

        @property
        def fig(self):
            return self.mpl_wgt.fig
