...
class QueryResultsWidget(gt.APIItemWidget):
    ...
    def __init__(qres_wgt, ibs, cm_list, parent=None, callback=None,
        ...
        if USE_FILTER_PROXY:
            super(QueryResultsWidget, qres_wgt).__init__(parent=parent, model_class=CustomFilterModel)
        else:
    ...
    def repopulate(qres_wgt):
        ...
        qres_wgt.qreq_ = qreq_
        qres_wgt.review_api = id_review_api.make_review_api(qres_wgt.ibs,
                                                            qres_wgt.cm_list,
                                                            qreq_=qres_wgt.qreq_,
                                                            review_cfg=qres_wgt.review_cfg)

        headers = qres_wgt.review_api.make_headers(tblname='review_api',
                                                   tblnice=tblnice)

        # HACK IN ROW SIZE
        vertical_header = qres_wgt.view.verticalHeader()
        vertical_header.setDefaultSectionSize(
            qres_wgt.review_api.get_thumb_size())
    ...
    def mark_unreviewed_above_score_as_correct(qres_wgt):
        ...
        if len(selected_qtindex_list) == 1:
            ...
            aids1 = qres_wgt.review_api.get(qres_wgt.review_api.col_name_list.index('qaid'), valid_rows)
            aids2 = qres_wgt.review_api.get(qres_wgt.review_api.col_name_list.index('aid'), valid_rows)
            #ibs = qres_wgt.ibs
            ibs = qres_wgt.ibs
        ...
        options += [
            ...
            ('New Graph Interaction (Names)',
             partial(viz_graph2.make_qt_graph_interface,
                     ibs, nids=ibs.get_annot_nids(aid_list2))),
        ]
