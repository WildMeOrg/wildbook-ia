...
def label_to_colors(labels_):
    ...
    unique_colors = df2.distinct_colors(len(unique_labels))
    label2_color = dict(zip(unique_labels, unique_colors))
    colors = [label2_color[label] for label in labels_]
    return colors


def plot_chip_metric(ibs, aid, metric=None, fnum=1, lbl='', colortype='score', **kwargs):
    """
    ...
    if metric is not None:
        if  colortype == 'score':
            ...
            colors = label_to_colors(metric)
        else:
            raise ValueError('no known colortype = %r' % (colortype,))
    else:
        colors = 'distinct'
    viz_chip.show_chip(ibs, aid, color=colors, darken=.5, **kwargs)
    if metric is not None:
...
def viz_annot_with_metrics(ibs, invindex, aid, metrics, metric_keys=['mean', 'std', 'min', 'max']):
    ...
    Example:
        ...
        >>> ax2_idxs = [np.where(invindex.idx2_daid == aid)[0] for aid in ut.progiter(invindex.daids, 'Building Forward Index: ', freq=100)]
    ...
    plot_chip_metric(ibs, aid, idf_list, fnum=1, lbl='idf', colortype='score')
    plot_chip_metric(ibs, aid, wxs, fnum=2, lbl='wx', colortype='label')
    ...
    plot_chip_metric(ibs, aid, metric=None, fnum=8, lbl='', annot=False)
...
def vizualize_vocabulary(ibs, invindex):
    ...
    (wx2_pdist, wx2_wdist, wx2_nMembers, wx2_pdist_stats, wx2_wdist_stats) = compute_word_metrics(invindex)
...
if __name__ == '__main__':
    ...
    #tup = smk_debug.testdata_raw_internals0(db='GZ_ALL', nWords=8000)
    #tup = smk_debug.testdata_raw_internals0(db='PZ_Master0', nWords=64000)
    tup = smk_debug.testdata_raw_internals0(db='testdb1')
    ibs, annots_df, daids, qaids, invindex, qreq_ = tup
    smk_repr.compute_data_internals_(invindex, qreq_.qparams, delete_rawvecs=False)
    invindex.idx2_wxs = np.array(invindex.idx2_wxs)
    aid = 3
    ax2_idxs = [np.where(invindex.idx2_daid == aid_)[0] for aid_ in ut.progiter(invindex.daids, 'Building Forward Index: ', freq=100)]
    metrics = compute_word_metrics(invindex)
    kwargs = {}
    #plot_chip_metric(ibs, aid, metric=None, fnum=1, lbl='', colortype='score', **kwargs)
    for aid in ibs.get_valid_aids():
        ...
        ans = input('press the "any key" to continue (press q to quit)')
        if ans == 'q':
            break
    exec(execstr)
