...
def _nn_normalized_weight(normweight_fn, qaid2_nns, qreq):
    #import utool
    #utool.stash_testdata('qaid2_nns')
    # Only valid for vsone
    K = qreq.qparams.K
    Knorm = qreq.qparams.Knorm
    rule  = qreq.qparams.normalizer_rule
    ...
    dx2_aid = qreq.indexer.dx2_rowid
    dx2_fx = qreq.indexer.dx2_fx
    for qaid in six.iterkeys(qaid2_nns):
        ...
        if rule == 'last':
            ...
            qnid = qreq.get_annot_nids(qaid)
            nTop = max(1, K)
            qfx2_topdx = qfx2_dx.T[0:nTop, :].T
            qfx2_normdx = qfx2_dx.T[-Knorm:].T
            # Apply temporary uniquish name
            qfx2_topnid = qreq.get_annot_nids(dx2_aid[qfx2_topdx])
            qfx2_normnid = qreq.get_annot_nids(dx2_aid[qfx2_normdx])
