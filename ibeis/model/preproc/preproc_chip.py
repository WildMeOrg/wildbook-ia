...
(print, print_, printDBG, rrr, profile) = utool.inject(__name__, '[preproc_chip]', DEBUG=False)
...
def get_chip_fname_iter(ibs, rid_list):
...
def get_chip_fpath_list(ibs, rid_list):
    cfname_iter = get_chip_fname_iter(ibs, rid_list)
    cfpath_list = [join(ibs.chipdir, cfname) for cfname in cfname_iter]
    return cfpath_list


def gen_chips_async(cfpath_list,
                    gfpath_list,
                    bbox_list,
                    theta_list,
                    newsize_list,
...
def compute_and_write_chips_lazy(ibs, rid_list):
    ...
    compute_and_write_chips(ibs, dirty_rid_list)
