...
def _nn_normalized_weight(normweight_fn, ibs, qcx2_nns, qreq):
    #import utool
    #utool.stash_testdata('qcx2_nns')
    ...
    for qcx in qcx2_nns.iterkeys():
        ...
        if rule == 'last':
            ...
            qtnx = ibs.cid2_tnx(qcx)
            nTop = max(1, K)
            qfx2_topdx = qfx2_dx.T[0:nTop, :].T
            qfx2_normdx = qfx2_dx.T[-Knorm:].T
            # Apply temporary uniquish name
            qfx2_toptnx = ibs.cid2_tnx(dx2_cx[qfx2_topdx])
            qfx2_normtnx = ibs.cid2_tnx(dx2_cx[qfx2_normdx])
...
def nn_bursty_weight(ibs, qcx2_nns, qreq):
...
def nn_recip_weight(ibs, qcx2_nns, qreq):
...
def nn_roidist_weight(ibs, qcx2_nns, qreq):
    ...
    for qcx in qcx2_nns.iterkeys():
        ...
        qfx2_kpts = cid2_kpts[qcx]
        nQuery = len(qfx2_dx)
        qfx2_cx = dx2_cx[qfx2_nn]
        qfx2_fx = dx2_fx[qfx2_nn]
        qfx2_chipsize2 = array([cid2_rchip_size[cid] for cid in qfx2_cx.flat])
        ...
        qdiag = np.sqrt((array(cid2_rchip_size[qcx]) ** 2).sum())
        qfx2_xy1 = array(qfx2_kpts[:, 0:2], np.float)
        qfx2_xy1[:, 0] /= qdiag
        qfx2_xy1[:, 1] /= qdiag
        # Get database relative xy keypoints
        qfx2_xy2 = array([cid2_kpts[cid][fx, 0:2] for (cid, fx) in
        ...
        cid2_roidist_weight[qcx] = qfx2_xydist
    return cid2_roidist_weight


def nn_scale_weight(ibs, qcx2_nns, qreq):
    # Filter by scale for funzies
    K = qreq.cfg.nn_cfg.K
    cid2_scale_weight = {qcx: None for qcx in qcx2_nns.iterkeys()}
    qcx2_metaweight = {qcx: None for qcx in qcx2_nns.iterkeys()}
    data_index = qreq._data_index
    K = qreq.cfg.nn_cfg.K
    cid2_kpts = ibs.feats.cid2_kpts
    ...
    for qcx in qcx2_nns.iterkeys():
        ...
        qfx2_det2 = array([cid2_kpts[cid][fx, [2, 4]] for (cid, fx) in
        ...
        cid2_scale_weight[qcx] = qfx2_scaledist
    return cid2_scale_weight, qcx2_metaweight
