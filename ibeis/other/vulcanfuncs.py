...
def vulcan_wic_validate(ibs, config_list, offset_black=0, target_recall_list=None,
                        ...
                        use_ancestors=False, quick=False, **kwargs):
    """
    Example:
        >>> config_list = [
    ...
    if quick:
        ...
        for index in range(len(config_list)):
            ...
            if classifier_algo in ['tile_aggregation']:
                classifier_algo = '%s_quick' % (classifier_algo, )
...
def vulcan_localizer_validate(ibs, target_species='elephant_savanna',
    ...
    config_dict = {
        'vulcan-gt-positive-margin-%s-v0-v1' % (margin, ): template_v0,
    }
...
def vulcan_localizer_image_validate(ibs, target_species='elephant_savanna',
    ...
    key = 'vulcan-localizer-image-%s-comparison-final' % (algo, )
    config_dict = {
        key: template_v0,
    }
...
@register_ibs_method
def vulcan_visualize_annotation_clusters(ibs, assignment_image_dict, tag,
                                         output_path=None, **kwargs):
    import plottool as pt
    import cv2

    if output_path is None:
        folder_name = 'bboxes_circles_%s' % (tag, )
    ...
    for gid in tqdm.tqdm(assignment_image_dict):
        ...
        write_filename = 'bboxes_%d_circle_annots_%d_clusters_%d.png' % (gid, len(index_list), len(cluster_list), )
...
@register_ibs_method
def vulcan_visualize_annotation_clusters_distribution(ibs, target_species='elephant_savanna',
                                                      min_cumulative_percentage=0.025,
                                                      **kwargs):
    import matplotlib.pyplot as plt
    import plottool as pt
