...
def ensure_correct_version(ibs, db, version_expected, schema_spec,
    ...
    """
    if version_expected == constants.BASE_DATABASE_VERSION:
    ...
    force_incremental = params.args.force_incremental_db_update or version < version_expected
    is_base_version = (version == constants.BASE_DATABASE_VERSION)
    ...
    version = db.get_db_version()
    if not utool.QUIET:
        print('[_SQL] Database version: %r | Expected version: %r ' % (version, version_expected))
    if version < version_expected:
        ...
        if not utool.QUIET:
            print('[_SQL] Database version updated (incrementally) to %r' % (version_expected))
