...
class IBEISController(object):
    ...
    def __init__(ibs, dbdir=None, ensure=True, wbaddr=None, verbose=True):
        ...
        #                       allow_override='warn')   # autogenerated key
    ...
    def set_annot_verts(ibs, aid_list, verts_list, delete_thumbs=True):
        ...
        ibs.db.set(ANNOTATION_TABLE, colnames, val_iter1, id_iter1, nParams=nParams)
        ...
        ibs.db.set(ANNOTATION_TABLE, colnames, val_iter2, id_iter2, nParams=nParams)
        if delete_thumbs:
    ...
    def get_annot_chip_rowids(ibs, annot_rowid_list,
        ...
        if all_configs:
            config_rowid = ibs.dbcache.get(
                ...
                id_colname='annot_rowid', eager=eager, nParams=nParams)
        else:
            config_rowid = ibs.get_chip_config_rowid()
    ...
    @getter_1to1
    def get_annot_cids(ibs, aid_list, ensure=True, all_configs=False, eager=True, nParams=None):
    ...
    @getter_1to1
    def get_annot_fids(ibs, aid_list, ensure=False, eager=True, nParams=None):
        cid_list = ibs.get_annot_cids(aid_list, ensure=ensure, eager=eager, nParams=nParams)
        fid_list = ibs.get_chip_fids(cid_list, ensure=ensure, eager=eager, nParams=nParams)
        return fid_list

    @utool.accepts_numpy
    @getter_1toM
    @cache_getter(ANNOTATION_TABLE, 'kpts')
    def get_annot_kpts(ibs, aid_list, ensure=True, eager=True, nParams=None):
        """
        Returns:
            kpts_list (list): annotation descriptor keypoints
        """
        fid_list  = ibs.get_annot_fids(aid_list, ensure=ensure, eager=eager, nParams=nParams)
        kpts_list = ibs.get_feat_kpts(fid_list, eager=eager, nParams=nParams)
        return kpts_list

    @getter_1toM
    def get_annot_vecs(ibs, aid_list, ensure=True, eager=True, nParams=None):
        """
        Returns:
            vecs_list (list): annotation descriptor vectors
        """
        fid_list  = ibs.get_annot_fids(aid_list, ensure=ensure, eager=eager, nParams=nParams)
        vecs_list = ibs.get_feat_vecs(fid_list, eager=eager, nParams=nParams)
        return vecs_list

    @getter_1toM
    def get_annot_fg_weights(ibs, aid_list, ensure=True, eager=True, nParams=None):
        """
        Forground Weights
        ...
        """
        fid_list  = ibs.get_annot_fids(aid_list, ensure=ensure, eager=eager, nParams=nParams)
        fgweight_list = ibs.get_feat_fg_weights(fid_list, eager=eager, nParams=nParams)
        return fgweight_list

    @getter_1to1
    def get_annot_num_feats(ibs, aid_list, ensure=False, eager=True, nParams=None):
        """
        Returns:
            nFeats_list (list): num descriptors per annotation
        """
        fid_list = ibs.get_annot_fids(aid_list, ensure=ensure, nParams=nParams)
    ...
    @getter_1to1
    def get_chip_fids(ibs, cid_list, ensure=True, eager=True, nParams=None):
        if ensure:
    ...
    def get_feat_kpts(ibs, fid_list, eager=True, nParams=None):
        """
        Returns:
            kpts_list (list): chip keypoints in [x, y, iv11, iv21, iv22, ori] format """
        kpts_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_keypoints',), fid_list, eager=eager, nParams=nParams)
        return kpts_list

    @getter_1toM
    #@cache_getter(FEATURE_TABLE, 'feature_vecs')
    def get_feat_vecs(ibs, fid_list, eager=True, nParams=None):
        """
        Returns:
            vecs_list (list): chip SIFT descriptors """
        vecs_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_vecs',), fid_list, eager=eager, nParams=nParams)
        return vecs_list

    @getter_1toM
    def get_feat_fg_weights(ibs, fid_list, eager=True, nParams=None):
        """
        Returns:
            fgweight_list (list): probability of being a forground keypoint
        """
        fgweight_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_forground_weight',), fid_list, eager=eager, nParams=nParams)
        return fgweight_list

    @getter_1to1
    #@cache_getter(FEATURE_TABLE, 'feature_num_feats')
    def get_num_feats(ibs, fid_list, eager=True, nParams=None):
        """
        Returns:
            nFeats_list (list): the number of keypoint / descriptor pairs """
        nFeats_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_num_feats',), fid_list, eager=True, nParams=None)
    ...
    def get_feat_featweight_rowids(ibs, fid_list,
        ...
        if all_configs:
            config_rowid = ibs.dbcache.get(
                ...
                id_colname=FEAT_ROWID, eager=eager, nParams=nParams)
        else:
            config_rowid = ibs.get_featweight_config_rowid()
