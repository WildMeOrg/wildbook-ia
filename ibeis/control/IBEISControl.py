...
class IBEISController(object):
    ...
    @ider
    def _get_all_known_nids(ibs, _key=constants.INDIVIDUAL_KEY):
    ...
    def set_annotation_names(ibs, aid_list, name_list=None, nid_list=None):
        ...
        if nid_list is None:
            ...
            name_list = [constants.KEY_DEFAULTS[constants.INDIVIDUAL_KEY]
            ...
            aid_list = [aid for aid, name in izip(aid_list, name_list) if name != constants.KEY_DEFAULTS[constants.INDIVIDUAL_KEY]]
            name_list = [name for name in name_list if name != constants.KEY_DEFAULTS[constants.INDIVIDUAL_KEY]]
            # Convert names into nids
            nid_list = ibs.add_names(name_list)
    ...
    def set_annotation_species(ibs, aid_list, species_list=None, nid_list=None):
        ...
        if nid_list is None:
            ...
            species_list = [constants.KEY_DEFAULTS[constants.SPECIES_KEY]
            ...
            # remove the relationships that have now been unnamed
            aid_list = [aid for aid, species in izip(aid_list, species_list) if species != constants.KEY_DEFAULTS[constants.SPECIES_KEY]]
            species_list = [species for species in species_list if species != constants.KEY_DEFAULTS[constants.SPECIES_KEY]]
            # Convert names into nids
    ...
    def get_name_nids(ibs, name_list, ensure=True):
        ...
        nid_list = ibs.get_labelids_from_values(name_list, ibs.key_ids[constants.INDIVIDUAL_KEY])
    ...
    def get_names(ibs, nid_list):
        ...
        assert all([key == ibs.key_ids[constants.INDIVIDUAL_KEY]
    ...
    def get_species(ibs, nid_list):
        ...
        # SQL query. Then augment the name list to distinguish unknown names
        key_rowid_list = ibs.get_label_keys(nid_list)
        assert all([key == ibs.key_ids[constants.SPECIES_KEY]
