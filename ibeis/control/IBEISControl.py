...
class IBEISController(object):
    ...
    def get_annot_chip_rowids(ibs, annot_rowid_list,
                              config_rowid=None, all_configs=False,
                              ensure=True, eager=True,
                              num_params=None):
        """
        AUTOGENERATED METHOD: get_annot_chip_rowids

        Args:
            annot_rowid_list (list):

        Returns:
            chip_rowid_list
        """
        if ensure:
            ibs.add_chips(annot_rowid_list)
        if config_rowid is None:
            config_rowid = ibs.get_chip_config_rowid()
        colnames = ('chip_rowid',)
        if all_configs:
            config_rowid = ibs.dbcache.get(
                CHIP_TABLE, ('chip_rowid',), annot_rowid_list,
                id_colname='annot_rowid', eager=eager, num_params=num_params)
        else:
            config_rowid = ibs.get_chip_config_rowid()
            where_clause = 'annot_rowid=? AND config_rowid=?'
            params_iter = ((annot_rowid, config_rowid,)
                           for annot_rowid in annot_rowid_list)
            chip_rowid_list = ibs.dbcache.get_where(
                CHIP_TABLE, colnames, params_iter, where_clause, eager=eager,
    ...
    def get_annot_vecs(ibs, aid_list, ensure=True, eager=True, num_params=None):
        ...
        """
        fid_list  = ibs.get_annot_fids(aid_list, ensure=ensure, eager=eager, num_params=num_params)
        vecs_list = ibs.get_feat_vecs(fid_list, eager=eager, num_params=num_params)
        return vecs_list
    ...
    def get_feat_vecs(ibs, fid_list, eager=True, num_params=None):
        ...
        vecs_list = ibs.dbcache.get(FEATURE_TABLE, ('feature_vecs',), fid_list, eager=eager, num_params=num_params)
        return vecs_list
