...
def add_test(ibs, test_challenge_list, test_response_list, test_result_list=None,
             test_uuid_list=None, test_user_identity_list=None):
    assert len(test_challenge_list) == len(test_response_list)
    n_input = len(test_challenge_list)

    test_challenge_list = [
        ut.to_json(test_challenge)
        for test_challenge in test_challenge_list
    ]

    test_response_list = [
        ut.to_json(test_response)
        for test_response in test_response_list
    ]
    ...
    superkey_paramx = (0,)
    colnames = [TEST_UUID, TEST_CHALLENGE, TEST_RESPONSE, TEST_RESULT,
                TEST_USER_IDENTITY]
    params_iter = list(zip(test_uuid_list, test_challenge_list, test_response_list,
                           test_result_list, test_user_identity_list))
    test_rowid_list = ibs.staging.add_cleanly(const.TEST_TABLE, colnames,
                                                params_iter,
...
def get_test_rowids_from_uuid(ibs, uuid_list):
    test_rowid_list = ibs.staging.get(const.TEST_TABLE, (TEST_ROWID,), uuid_list,
                                      id_colname=TEST_UUID)
    return test_rowid_list
