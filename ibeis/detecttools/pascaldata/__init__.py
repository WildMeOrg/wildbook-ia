...
class PASCAL_Data(object):
    ...
    def __str__(pascald):
        return "<IBEIS Data Object | %s | %d images | %d categories | %d rois>" \
            % (pascald.absolute_dataset_path, len(pascald.images), len(pascald.categories), pascald.rois)
    ...
    def print_distribution(pascald):
        ...
        _max = max([ len(category) for category in pascald.distribution_rois.keys() + ['TOTAL', 'CATEGORY'] ]) + 3
        ...
        for category in sorted(pascald.distribution_rois):
            _print_line(category, _max, pascald.distribution_images[category], pascald.distribution_rois[category])

        _print_line("TOTAL", _max, len(pascald.images), pascald.rois)

    def dataset(pascald, positive_category, neg_exclude_categories=[], max_rois_pos=None, max_rois_neg=None):
        def _parse_dataset_file(category, _type):
            filepath = os.path.join(pascald.dataset_path, "ImageSets", "Main", category + "_" + _type + ".txt")
            _dict = {}
            try:
                _file = open(filepath)
                for line in _file:
