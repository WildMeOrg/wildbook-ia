...
def update_1_3_6(db, ibs=None):
    ...
    db.modify_table(const.ANNOTATION_TABLE, shortname='annot')
...
def update_1_3_8(db, ibs=None):
    ...
    db.modify_table('encounters', superkeys=[('encounter_text',)])
...
def update_1_4_5(db, ibs=None):
    db.modify_table(const.ANNOTMATCH_TABLE, [(None, 'annotmatch_note', 'TEXT', None)])
...
def update_1_5_1(db, ibs=None):
    # Rename encounters to imagesets
    db.modify_table(const.IMAGE_TABLE, superkeys=[(IMAGE_UUID,)])
    db.modify_table(
        const.ANNOTMATCH_TABLE,
        superkeys=[('annot_rowid1', 'annot_rowid2')],
    )
