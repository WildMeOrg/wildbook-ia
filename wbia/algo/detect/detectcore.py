...
def export_annots_to_coco(
    ...
    **kwargs,
    ...
    for (
        ...
        logger.info(
            'Copying:\n%r\n%r\n%r\n\n' % (old_filepath, target_fpath, (width, height))
        )
        # TODO: could just do this in one call up top
        ut.copy_files_to([old_filepath], dst_fpath_list=[target_fpath])
...
@register_ibs_method
def train_split_by_name_naive(ibs, aid_list, split_frac=0.8, rand_seed=None):
    import random, math
    ...
    name_freqs = ut.dict_hist(name_list)
    name_set = set(name_list)
    train_names = random.sample(name_set, math.floor(split_frac * len(name_set)))
    train_aids = [aid for aid, name in zip(aid_list, name_list) if name in train_names]
    test_aids = [aid for aid, name in zip(aid_list, name_list) if name not in train_names]

    return train_aids, test_aids


@register_ibs_method
def train_split_by_name(ibs, aid_list, split_frac=0.8, rand_seed=None, bin_size=10):
    assert 0 <= split_frac <= 1, 'split_frac must be between 0 and 1'
    import random, math
    ...
    names.sort(key=lambda name: name_freqs[name])

    train_names = set()
    test_names = set()
    bin_split_index = math.floor(split_frac * bin_size)
    for bin_no in range(0, math.ceil(len(names) / bin_size)):
        bin_names = names[bin_no * bin_size : (bin_no + 1) * bin_size]
        random.shuffle(bin_names)
        train_names.update(bin_names[:bin_split_index])
        test_names.update(bin_names[bin_split_index:])

    train_aids = [aid for aid, name in zip(aid_list, name_list) if name in train_names]
    test_aids = [aid for aid, name in zip(aid_list, name_list) if name in test_names]

    return train_aids, test_aids


def print_split_summary(ibs, train_aids, test_aids, split_frac=0.8):
    import statistics

    total_aids = len(train_aids) + len(test_aids)
    real_frac = len(train_aids) / total_aids
    split_error = (abs(split_frac - real_frac) / split_frac) * 100
    print(
        f'{total_aids} split into {len(train_aids)} train_aids and {len(test_aids)} test aids:'
    )
    print(
        f'split_frac = {split_frac} goal vs. {real_frac:.4f} result. Error = {split_error:.3f}%'
    )

    train_name_list = ibs.get_annot_names(train_aids)
    train_name_freqs = ut.dict_hist(train_name_list).values()
    train_aids_per_name = statistics.mean(train_name_freqs)
    train_apm_stdev = statistics.stdev(train_name_freqs)
    print(
        f'TRAIN: {train_aids_per_name:.2f} aids per name, stdev = {train_apm_stdev:.2f}'
    )
    test_name_list = ibs.get_annot_names(test_aids)
    test_name_freqs = ut.dict_hist(test_name_list).values()
    test_aids_per_name = statistics.mean(test_name_freqs)
    test_apm_stdevf = statistics.stdev(test_name_freqs)
    print(f'TEST:  {test_aids_per_name:.2f} aids per name, stdev = {test_apm_stdevf:.2f}')
...
@register_ibs_method
def imageset_id_training_split(ibs, aid_list, train_split=0.8):
    ...
    train_gids = [
        gid
        for gid, name in zip(gid_list, names)
        if name in train_names and name is not '____'
    ]
    test_gids = [
        gid
        for gid, name in zip(gid_list, names)
        if name in test_names and name is not '____'
    ]
