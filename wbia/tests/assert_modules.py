        ...
        def checkinfo_wrapper(*args, **kwargs):
            ...
            try:
                ...
                return (
                    ...
                    infodict,
                    ut.formatex(ex),
                    'Some unknown error in ' + packagename,
                )
...
def module_stdinfo_dict(
    ...
    infodict = {
        '__version__': version
        if module is None or version is not None
        else getattr(module, versionattr, None),
...
def check_modules_exists():
    ...
    for modname in modname_list:
        try:
            ...
            ut.printex(
                ex,
                'Some othere error happened when importing %r ' % (modname,),
                iswarning=True,
            )
            failed_list.append(modname)
    ...
    for checkinfo_wrapper in ASSERT_FUNCS:
        ...
        if passed:
            statustext_list.append(
                funcname + ' ' + str(infodict['__version__']) + ' passed'
            )
            # statustext_list.append('')
        else:
...
if __name__ == '__main__':
    ...
    # if len(sys.argv) == 1:
    if not any(
        argv.startswith('--test-') or argv.startswith('--exec') for argv in sys.argv
    ):
        assert_modules()
    else:
        ut.doctest_funcs()
