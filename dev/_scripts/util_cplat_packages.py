...
def version_ge(version1, version2):
    ...
    flag = distutils.version.LooseVersion(version1) >= distutils.version.LooseVersion(
        version2
    )
    return flag
...
def __install_command_macports(pkg):
    ...
    if pkg == 'python':
        ...
        pyselect_set2 = (
            'sudo select python python' + MACPORTS_PY_SUFFIX + ' @' + TARGET_PY_VERSION
        )
...
def apply_preinstall_fixes():
    if FEDORA_FAMILY:
        ...
        return [
            cmd(command, lbl='_fix_yum_repos: ' + str(count))
...
def ensure_package(pkg):
    ...
    if DEBIAN_FAMILY:
        ...
        raise NotImplementedError(
            '%r is not yet supported' % ((__OS__, distro, distro_version,),)
        )
    return cmd(command)
...
def make_prereq_script(
    ...
    if with_pypkg:
        output_list.extend(
            [ensure_python_package(pypkg, upgrade=upgrade) for pypkg in pypkg_list]
        )
    if with_config:
